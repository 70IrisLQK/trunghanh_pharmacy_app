...
class Cart {
  ...
  getTotalCartCount(cartList, count) {
  ...
  getItemCountByProductView(cartList, cartItem) {
    if (cartList !== undefined && cartList !== null && cartList.length > 0) {
      let itemData = cartList.filter(item => item.id === cartItem.product_id);
      if (itemData !== undefined && itemData !== null && itemData.length > 0) {
        return itemData[0].count;
      } else {
        return 0;
  ...
  isProductExist(cartList, cartItem) {
    if (cartList !== undefined && cartList !== null && cartList.length > 0) {
      let itemIndex = cartList.findIndex(item => item.id === cartItem.id);
      if (itemIndex === -1) {
        return -1;
      } else {
        return itemIndex;
  ...
  isProductViewExist(cartList, cartItem) {
    if (cartList !== undefined && cartList !== null && cartList.length > 0) {
      let itemIndex = cartList.findIndex(item => item.id === cartItem.product_id);
      if (itemIndex === -1) {
        return -1;
      } else {
        return itemIndex;
